...
<body>
    ...
    <p><b>Name : </b><?php echo $element['username'] ?></p>
    ...
    <div class="form" id="form">
        ...
        <form action="transfer.php" method ="post" onsubmit="return check_amount()">
            <br>
            <input type="text" name="u1" id='u1' value="<?php echo $element['username'] ?>" style="display: none;">
            <label style="color: whitesmoke; margin: 20px;">Transfer to :</label>
            <select name="u2" id="u2" required>
                <option value="">--Select a user--</option>
                <?php
                $sql_query2 = "select * from users where username != '$name'";
                $result2 = mysqli_query($conn,$sql_query2);
                $noRow= mysqli_num_rows($result2);
                for ($i=0; $i < $noRow; $i++) {
                    $element2 =mysqli_fetch_assoc($result2);
                    echo "<option value=".$element2['username'].">".$element2['username']."</option>";
                }
                ?>
            </select>
            <br>
